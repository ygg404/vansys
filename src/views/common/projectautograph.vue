...
<div class="main-navbar">
 ...
 <van-row type="flex" justify="center" style="margin-top:10px;" class="textstyle">
  <van-col span="5" >项目编号:</van-col>
  <van-col span="18" >{{dataForm.projectNo}}</van-col>
 </van-row>

  <van-row type="flex" justify="center" style="margin-top:5px;" class="textstyle">
  <van-col span="5" >项目名称:</van-col>
  <van-col span="18" >{{dataForm.projectName}}</van-col>
 </van-row>
  ...
  <div>
    ...
    <van-row type="flex" justify="center">
      <van-col span="8"><van-button @click="cleanText()" style="color:red;">清除</van-button></van-col>
      <van-col span="8"><van-button @click="commitData()" style="color:#667AFA;">提交签名</van-button></van-col>
    </van-row>
...
import wx from 'weixin-js-sdk'
  export default {
    ...
    //得到URL地址中的projectNo
    var projectNoStr = part.substring(projectStrNum, projectStrNum.length)
    //传projectNo 请求后台 得到项目信息用于显示 
    // this.getProjectInfo(projectNoStr)
     this.getProjectInfo(projectNoStr).then(success => {
              this.judgeAutograph()
            })
    //   alert("参数" + projectNoStr)
    },
    mounted () {
    ...
    methods: {
      getProjectInfo(projectNo){
        var urlInfo = '/project/projectInfo/info/' + projectNo
        return new Promise((resolve,reject) =>{
          this.$http({
            url: this.$http.adornUrl(urlInfo),
            method: 'get',
            ...
            if(data && data.code === 0){
              ...
              this.dataForm.projectNo = info.projectNo
              this.dataForm.createUserName = info.createUserName
              this.dataForm.createUserId = info.createuserid
              this.dataForm.crateTime = new Date()
              console.log(info)
               resolve(data)
            }else{
        ...
        let that = this
          ...
          lrz(that.$refs.board.toDataURL(), options)
            .then(function (rst) {
               ...
               that.dataForm.sigImage = that.imgUrl
            }).catch(function (error) {
          }).always(function () {
              that.commitProjectAutographData().then(success => {
              //that.DownloadAutographExcel()
            })
          })  
      },

      //判断该项目是否在数据库中提交过 project_no
      judgeAutograph () {
        var that = this
        return new Promise((resolve, reject) => {
           this.$http({
           url: this.$http.adornUrl('/project/projectarchives/list'),
           method: 'get',
           params: this.$http.adornParams({})
        }).then(({data}) => {
            if (data && data.code === 0) {
            var commitedprojectNoList = data.list
            if(commitedprojectNoList.includes(that.dataForm.projectNo) == true){
              that.canCommitFlag = 'update'
            }
              resolve(data)
            } else {
              this.$message.error(data.msg)
        ...
        return new Promise((resolve, reject) => {
          ...
          method: 'post',
           data: this.$http.adornData({
                ...
                createTime:this.dataForm.crateTime
              })
        }).then(({data}) => {
            if (data && data.code === 0) {
              resolve(data)
