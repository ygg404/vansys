<template>
  <van-dialog
    ...
    @confirm="dataFormSubmit"
              v-model="visible">
    <template slot="title">
      <div style="margin-bottom:10px;text-align:center;font-size:16px;font-weight:600;">个人加减分编辑（  被考核人：{{checkUserItem.userName}} ）
      </div>
    </template>
    <div style="height:500px;overflow: scroll;" class="extratable">
      <el-table :data="extraScoreList" border :span-method="objectSpanMethod" align="center" show-summary>
        <el-table-column prop="extraType" label="类型" width="40px">
        ...
        <el-table-column prop="extraNum" label="分数" width="150px">
          <template slot-scope="scope">
            <el-input-number v-model="scope.row.extraNum" size="mini"
