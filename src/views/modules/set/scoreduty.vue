  ...
  export default {
    ...
    methods: {
      ...
      deleteHandle (item) {
        ...
        this.$dialog.confirm({
          title: '提示',
          message: `确定对职务[${item.duty}]进行[${ids ? '删除' : '批量删除'}]操作?`
        }).then(() => {
          this.$http({
            url: this.$http.adornUrl('/set/scoreduty/delete'),
            method: 'post',
      ...
      datadragEnd (evt) {
        ...
        this.$http({
          url: this.$http.adornUrl('/set/scoreduty/changeSort'),
          method: 'get',
          params: this.$http.adornParams({
...
<style scoped>
  .scoreduty_table table tr th,.scoreduty_table table tr td{
    border: 1px solid #ccc;
  }
  .scoreduty_table table{
